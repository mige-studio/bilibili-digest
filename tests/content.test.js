...
test('真实页面适配只读取当前分 P，并选择可转写的 B 站 AAC 音轨',async()=>{
  ...
  const w=dom.window,video=w.document.querySelector('video');Object.defineProperty(video,'duration',{value:120});Object.defineProperty(video,'currentTime',{value:0,writable:true});video.getBoundingClientRect=()=>({left:40,top:120,right:1040,bottom:680,width:1000,height:560});video.play=async()=>{};
  ...
  try{
    ...
    const entry=w.document.querySelector('#bilid-entry');assert.equal(entry.style.top,'120px');assert.equal(entry.style.left,'40px');assert.equal(entry.style.width,'1000px');assert.match(script,/B站精读<\/button><button[^>]+>记笔记/);assert.doesNotMatch(script,/bottom:86px|记下当前/);
