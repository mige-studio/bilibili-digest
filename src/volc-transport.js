...
const BILI_VOLC = (() => {
 ...
 const localReasons={
  configuration:'插件识别版本未正确连接，请重新加载“B站精读”。',
 ...
 async function recordingBody(audio,request) {
  ...
  const parts=[JSON.stringify({user:{uid:'bilibili-digest'},request}).slice(0,-1)+',"audio":{"data":"'];
 ...
 async function request(action,key,job,mediaUrl,fetcher=fetch) {
  ...
  try {
   ...
   if(action==='submit'){
    ...
    body={user:{uid:'bilibili-digest'},audio,request:{model_name:'bigmodel',enable_punc:true,enable_itn:true,show_utterances:true,enable_speaker_info:true}};
 ...
 async function flash(key,jobId,bytes,fetcher=fetch,speakers=false) {
  ...
  const response=await fetcher('https://openspeech.bytedance.com/api/v3/auc/bigmodel/recognize/flash',{
   ...
   body:JSON.stringify({user:{uid:'bilibili-digest'},audio:{data:btoa(binary)},request:{model_name:'bigmodel',enable_punc:true,enable_itn:true,show_utterances:true,...(speakers?{enable_speaker_info:true}:{})}}),signal:AbortSignal.timeout(600000)});
  const code=response.headers.get('X-Api-Status-Code');
