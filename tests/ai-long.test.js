...
test('长稿完整传递到最后一句，沿用8192输出及关闭推理的母版请求',async()=>{
 ...
 globalThis.fetch=async(url,options)=>{const body=JSON.parse(options.body);assert.equal(url,'https://ark.cn-beijing.volces.com/api/v3/chat/completions');assert.equal(body.messages[1].content,transcript);assert.equal(body.max_tokens,8192);assert.equal(body.thinking.type,'disabled');return new Response(JSON.stringify({choices:[{message:{content:'{"chapters":[],"keyQuotes":[]}'}}]}));};
